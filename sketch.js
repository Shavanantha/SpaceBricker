...
class TrapBossHealEffect { 
    apply(_balls, game) { 
        game.bricks.forEach(b => b.heal(10)); 
    } 
}
...
const POWER_EFFECTS = {
  ...
  6: new TrapLifeEffect(), 
  7: new TrapBossHealEffect() 
};
...
class Renderer {
  ...
  // FIX: Tampilan Menu Utama Diperbarui Jadi Lebih Menarik!
  drawMenu(highScore) {
    fill(0, 180); rect(0, 0, width, height); 
    
    // Efek glowing bernapas untuk judul
    let glow = map(sin(frameCount * 0.05), -1, 1, 10, 40);
    drawingContext.shadowBlur = glow; 
    drawingContext.shadowColor = 'cyan';
    fill(0, 255, 255); textAlign(CENTER); textSize(55); textStyle(BOLD); 
    text("SPACE BRICKER ", width / 2, height / 4 - 30);
    
    drawingContext.shadowBlur = 0; textSize(20); fill(255);
    text("🏆 HIGH SCORE: " + highScore + " 🏆", width / 2, height / 4 + 25);
    
    buttons.forEach(btn => btn.display());
    
    // Panel Info Kiri
    this._drawInfoBox(width / 2 - 320, height / 2 + 80, 300, 220, "🎮 CARA BERMAIN", [
        "⬅️ ➡️ : Gerak Kanan / Kiri", 
        "🧱 : Hancurkan Bata", 
        "💀 : Jangan Biarkan Bola Jatuh!"
    ]);
    
    // Panel Info Kanan
    this._drawInfoBox(width / 2 + 20, height / 2 + 80, 300, 220, "🪐 PLANET ABILITIES", [
        "🔥 Api  | ❄️ Es  | 🌀 Multi-ball", 
        "🟢 Hijau: Giant Ball",
        "💗 PINK(+): +1 Nyawa", 
        "⚠️ TRAP (LEVEL 3):", 
  ...
  _drawInfoBox(x, y, w, h, title, lines) {
    push(); 
    // Kotak bergaya Sci-Fi HUD
    drawingContext.shadowBlur = 15; 
    drawingContext.shadowColor = 'rgba(0, 255, 255, 0.4)';
    fill(10, 20, 40, 230); 
    stroke(0, 255, 255, 150); strokeWeight(2); 
    rect(x, y, w, h, 15);
    
    drawingContext.shadowBlur = 0;
    fill(0, 255, 255); textAlign(CENTER); textStyle(BOLD); textSize(18); 
    ...
    fill(230); textStyle(NORMAL); textSize(14); textAlign(LEFT); noStroke();
    for (let i = 0; i < lines.length; i++) { 
        if (lines[i].includes("⚠️")) fill(255, 100, 100); // Highlight merah untuk peringatan
        text(lines[i], x + 20, y + 75 + i * 25); 
    }
    pop();
  }
}
...
class PowerUp extends IMovable(IDisplayable(GameObject)) {
  constructor(x, y, pType) {
    ...
    this.pColor = [
        color(0, 255, 100),   // 1 (Hijau/Giant)
        color(0, 200, 255),   // 2 (Biru/Multi)
        color(255, 100, 0),   // 3 (Oranye/Api)
        color(100, 200, 255), // 4 (Biru Muda/Es)
        color(255, 50, 100),  // 5 (Pink + Nyawa)
...
class Game {
  ...
  spawnMinion() {
    ...
    let minion = new Brick(random(50, width - 50), random(height / 2, height - 200), 75, 25, hpRandom, false);
    minion.setLevel(this.level);
    this.bricks.push(minion);
  }

  getBalancedPowerUpType() {
      let pool = [1, 2, 3, 4, 5]; 
  ...
  update() {
    ...
    for (let i = this.balls.length - 1; i >= 0; i--) {
      ...
      if (hitResult) {
        ...
        if (hitResult.destroyed || hitResult.isBoss) {
            let dropChance = 0.35; 
            
            if (random(1) < dropChance) { 
                let pType = this.getBalancedPowerUpType();
                
                // FIX BUG: Trap tidak dihitung sebagai kuota 5 power-up. Hanya planet baik (tipe 1-5) yang dibatasi!
                if (pType <= 5) {
                    if (this.powerUpsSpawnedThisLevel < 5) {
                        this.powerUps.push(new PowerUp(hitResult.brickX, hitResult.brickY, pType)); 
                        this.powerUpsSpawnedThisLevel++; 
                    }
                } else {
                    // Planet Trap (6 & 7) boleh keluar kapanpun sebagai hukuman
                    this.powerUps.push(new PowerUp(hitResult.brickX, hitResult.brickY, pType));
                }
            }
        }
...
function setup() {
  ...
  if (breakSound) breakSound.playMode('restart');
  if (powerSound) powerSound.playMode('restart');
